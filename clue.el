...
(defcustom clue-keymap
  (let ((map (make-sparse-keymap)))
    (define-key map (kbd "RET") 'clue-follow-clue)
    (define-key map (kbd "<mouse-1>") 'clue-follow-clue)
    map)
  "Keymap that's enabled on clues."
  :type 'keymap)

;;;; Internals

(defvar clue--clue-regexp
  ;; generated by (rx "#[" (zero-or-more (not (or "[" "]"))) "]")
  "#\\[[^][]*]"
  "Regexp to match clues.")

(defun clue--unfontify (beg end)
  "Remove fontification of clues between BEG and END."
  (dolist (ov (overlays-in beg end))
    (when (overlay-get ov 'cluep)
      (delete-overlay ov))))

(defun clue--fontify (beg end)
  "Fontify the clues between BEG and END."
  (save-excursion
    (goto-char beg)
    (while (re-search-forward clue--clue-regexp end t)
      (let* ((b (match-beginning 0))
             (e (match-end 0))
             ov)
        (setq ov (make-overlay b e))
        (overlay-put ov 'cluep t)
...
(defun clue--refontify (beg end)
  "Refontify the clues between BEG and END.
...
(defun clue-follow-clue ()
  "Follow the clue under point."
  (interactive)
  (message "Jump!"))
